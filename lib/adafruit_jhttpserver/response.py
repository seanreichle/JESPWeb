...
class FileResponse(Response):  # pylint: disable=too-few-public-methods
    ...
    def _send(self) -> None:
        #print ("TYPE:" + str(self._content_type))
        led.value = 1
        if not self._head_only:
            if self._content_type == "application/j-python":
                #print ("Executing: " + str(self._full_file_path))            
                global jpyOutput, jpyRawRequest, jpyRequest, jpyHeader, jpyFile, jpyCode
                jpyOutput = ""
                jpyHeader = self._headers
                jpyRawRequest = self._request.raw_request
                jpyRequest = self._request.query_params
                jpyFile = self._full_file_path
                jpyCode = ""
                with open(jpyFile, "rt") as file:                
                    while bytes_read := file.read(self._buffer_size):
                        jpyCode += bytes_read
                    try:
                        exec(jpyCode)
                    except:
                        jpyOutput = "<h2>Code execution error</h2>"
                        
                    self._body = str(jpyOutput)
                    self._file_length = len(self._body)
